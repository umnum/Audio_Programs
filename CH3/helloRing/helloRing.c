...
void init_stuff()
{
	...
	{
		info = Pa_GetDeviceInfo(i); /* get information from
		                               current device */
		hostapi = Pa_GetHostApiInfo(info->hostApi); /* get info from
		                                               curr. host API */
	...
	outputParameters.hostApiSpecificStreamInfo = NULL; /* no specific
	                                                      info */
	for (i=0; i < Pa_GetDeviceCount(); i++)
	{
		info =  Pa_GetDeviceInfo(i); /* get information from
		                             current device */
		hostapi = Pa_GetHostApiInfo(info->hostApi); /* info from
		                                               host API */ 
...
void terminate_stuff()
{
	...
	Pa_CloseStream(audioStream); /* destroy the audio stream
	                                object */
	Pa_Terminate();
}
